    ...
    return this.each(function(){
      ...
      $.extend(setting, options, (this.getAttribute('data-overlayid'))? {id:this.getAttribute('data-overlayid')} : {})
			...
			if (setting.dir=="up" || setting.dir=="down"){ //set initial position of overlay box (either "top" or "left" property affected)
				startpoint={top:$target.outerHeight()* (setting.dir=="down"? -1 : 1) - 15}
			}
			else{
				startpoint={left:$target.outerWidth()* (setting.dir=="right"? -1 : 1) - 15}
			}
			...
			$overlay.css($.extend({position:'absolute', zIndex:'99', left:0, bottom:72, visibility:'visible'}, startpoint)).appendTo($target)
...
(function($){
  $.fn.overlaycontent2=function(options) {
               
    var setting={id:'', dir:'up', speed:200, opacity:1}
    return this.each(function(){
      $.extend(setting, options, (this.getAttribute('data-overlayid2'))? {id:this.getAttribute('data-overlayid2')} : {})
			var $target=$(this).css({position:'relative', overflow:'hidden'}) //set anchor DIV position to relative
			var $overlay=$('#'+(setting.id)) //reference overlay DIV box
			var startpoint={}
			if (setting.dir=="up" || setting.dir=="down"){ //set initial position of overlay box (either "top" or "left" property affected)
				...
				startpoint={left:$target.outerWidth()* (setting.dir=="right"? -1 : 1) - 0}
			}
			var overlaydimenions={w:$target.outerWidth()-($overlay.outerWidth()-$overlay.width()), h:$target.outerHeight()-($overlay.outerHeight()-$overlay.height())}
			$overlay.css($.extend({position:'absolute', zIndex:'99', width:overlaydimenions.w, left:0, bottom:0, visibility:'visible'}, startpoint)).appendTo($target)
			if (setting.opacity<1)
				$overlay.css({opacity:setting.opacity})
			$target.hover(
