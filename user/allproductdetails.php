	...
	$sql2=mysql_query("select `brand_name` from `brand` where `id`='$result1[brand_id]'")or die(mysql_error());
			...
			?>																																																						                                                                 <tr>
							...
							<div  style="float:left;">
												...
												{
												$que=mysql_query("select `description` from product_varient WHERE  `product_id`='$rwe[id]' and  `varient_name`='$v1' and `description`!=''")or die (mysql_error());
...
while($row1 = mysql_fetch_array($que))
  ...
  {		 
		...
		?>
		 <tr style="border-bottom:1px solid #cccccc; border-collapse:collapse; "><td class="para" style="width:200px;" ><a href="productdetails.php?idval=<?php echo $_GET['idval']?>&type=brand&bid=<?php echo $_GET['bid']?>&desc=<?php echo $row1['description'];?>&feature=<?php echo $v1;?>"><?php echo $row1['description'];?>&nbsp; </a></td></tr>
		 <?php
		 }
		 }
